...
{
    public class FrontEndAppContextService : AppContextServiceBase
    {
        private readonly ILogger<FrontEndAppContextService> _logger;
        ...
        public int ExternalFrontendPort { get; }
        public string ListenHost { get; }

        public FrontEndAppContextService(
            ILogger<FrontEndAppContextService> logger,
        ...
        ) : base(logger)
        {
            ...
            ExternalFrontendPort = configuration.GetValue("ExternalFrontendPort", appSettings.Value.ExternalFrontendPort);            
            ...
            //LoginType = configuration.GetValue("LoginType", appSettings.Value.LoginType);
            //UseNFT = configuration.GetValue("UseNFT", appSettings.Value.UseNFT);
            //Add2ndWalletLogin = configuration.GetValue("Add2ndWalletLogin", appSettings.Value.Add2ndWalletLogin);            
            //UseIAP = configuration.GetValue("UseIAP", appSettings.Value.UseIAP);

        }

        public override async Task StartAsync()
        {
            using (_logger.BeginScope("FrontEndAppContextService"))
            {
                _logger.LogInformation("----------------FrontEndAppContextService----------------");
                ...
                _logger.LogInformation("ExternalFrontendPort: {ExternalFrontendPort}", ExternalFrontendPort);                
